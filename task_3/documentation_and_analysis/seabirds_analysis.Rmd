...
```{r}
# 1. gives you warnings but produces expected result
iris %>% 
  separate(Species,
           into = c(NA, "after_t"),
           sep = "t", remove = FALSE)
...
# 2. can drop unwanted columns with dplyr::select()
iris %>% 
  separate(Species,
           into = c("before_t", "after_t"),
           sep = "t", remove = FALSE) %>% 
  select(-before_t)

# 3. how I'd actually do it (maybe incorrectly): (strings + regex is tomorrow)

iris %>% 
  mutate(after_t = str_extract(Species, "(?<=t)(.*)$"))
```
