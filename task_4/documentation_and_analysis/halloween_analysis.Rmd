...
respondent_info <- read_csv(
  here("data/clean_data/respondent_info.csv"),
  col_types = cols(sex = col_character(), country = col_character()
))

candy_info <- read_csv(
  here("data/clean_data/candy.csv")
)
```
...
```{r}
average_age_extroverts <- respondent_info %>% 
...
candy_info %>% 
  ...
  summarise(n_ratings = n(), .groups="drop_last") %>% 
  slice_max(n_ratings)
```
...
```{r}
saddened_by_starburst <- candy_info %>% 
  filter(rating == "DESPAIR",
         candy == "Starburst") %>% 
  nrow()
```
...
```{r}
favourites_by_sex <- respondent_info %>% 
  select(id, sex) %>% 
  left_join(candy_info, by = "id") %>% 
  select(sex, candy, rating) %>%
